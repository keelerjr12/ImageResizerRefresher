...
namespace ImageNS {
  ...
  bool in_bounds(const Image& img, int row, int col);
  ...
  Image::Image(int width, int height) : width(width), height(height),
    red_channel(width, height), green_channel(width, height),
  ...
  void Image::fill(const Pixel& color) {
    MatrixNS::fill(red_channel, color.r);
    MatrixNS::fill(green_channel, color.g);
    MatrixNS::fill(blue_channel, color.b);
  }
  ...
  Image from_stream(std::istream& is) {
    std::string format;
    is >> format;
  ...
  //           for an example.
  void print(const Image& img, std::ostream& os) {
    os << "P3" << std::endl;
  ...
  bool in_bounds(const Image& img, int row, int col) {
    if (row < 0 || row >= img.get_height())
      return false;
    if (col < 0 || col >= img.get_width())
      return false;
